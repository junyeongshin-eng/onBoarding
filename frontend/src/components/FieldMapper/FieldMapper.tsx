...
import type { CRMField, FieldMapping, SalesmapField, RecommendedField, ColumnToKeep } from '../../types';
...
interface FieldMapperProps {
  ...
  columnsToKeep?: ColumnToKeep[]; // 컨설팅에서 유지 추천된 컬럼 (초기 매핑용)
}
...
// Check if column is read-only (cannot be imported - auto-generated by system)
const isReadOnlyColumn = (columnName: string): boolean => {
  ...
  // Check for update date patterns (수정 날짜 is read-only, 생성 날짜 is editable)
...
export function FieldMapper({
  ...
  useEffect(() => {
    const loadFields = () => {
      ...
      // Auto-map fields using consulting result (columnsToKeep) first, then fallback to name matching
      if (mappings.length === 0 && allFieldsWithCustom.length > 0) {
        const autoMappings: FieldMapping[] = [];

        sourceColumns.forEach((col) => {
          // 1. First, try to use consulting result (columnsToKeep)
          const consultingMatch = columnsToKeep.find(c => c.columnName === col);
          if (consultingMatch && consultingMatch.targetObject && consultingMatch.targetField) {
            // Extract field name from "Lead - 이름" format
            const fieldLabelParts = consultingMatch.targetField.split(' - ');
            const fieldLabel = fieldLabelParts.length > 1 ? fieldLabelParts[1] : consultingMatch.targetField;

            // Find matching salesmap field by label
            const objType = consultingMatch.targetObject;
            const availableFields = salesmapFields[objType] || [];
            const matchingField = availableFields.find(f =>
              f.label === fieldLabel ||
              f.label === consultingMatch.targetField ||
              f.id === fieldLabel.toLowerCase()
            );

            if (matchingField) {
              autoMappings.push({
                source_column: col,
                target_field: `${objType}.${matchingField.id}`,
              });
              return;
            }
          }

          // 2. Fallback: match by column name
  ...
  }, [objectTypes, customFields.length, salesmapFields, recommendedFields, columnsToKeep]);
