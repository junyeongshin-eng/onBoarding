...
## 핵심 원칙: 데이터 보존 우선!
**가능한 모든 컬럼을 이관합니다.** 제외는 정말 불필요한 경우만!
사용자가 관리하던 모든 데이터는 중요합니다. 확실하지 않으면 이관하세요.

## 분류 목표 - 대부분 유지 (columns_to_keep)
다음은 모두 **유지**해야 합니다:
- 이름, 이메일, 전화번호, 회사명, 담당자 등 모든 연락처/조직 정보
- 출처, 상태, 단계, 채널 등 분류/추적 정보
- 금액, 수량, 개월 수 등 숫자 데이터
- 날짜, 타임스탬프 등 시간 정보
- 메모, 설명, 비고 등 텍스트 정보
- 커스텀 필드 (XX_로 끝나도 데이터가 있으면 유지)
- 확신이 없으면 유지!

## 제외 대상 - 극히 제한적으로만 (columns_to_skip)
아래 경우만 제외합니다:
1. **시스템 자동생성 ID**: 숫자만 있는 내부 식별자 (id, _id, seq 등)
2. **100% 비어있는 컬럼**: 모든 행에 값이 전혀 없음
3. **정확히 동일한 중복 컬럼**: 다른 컬럼과 값이 완전히 같음
4. **수정 불가 시스템 컬럼**: 수정 날짜, updated_at 등 (세일즈맵에서 자동 갱신됨)

주의:
- 생성 날짜, 담당자는 수정 가능하므로 제외하지 마세요!
- 빈 값이 일부 있어도 데이터가 있으면 유지! 80%가 비어있어도 20%는 중요할 수 있음
