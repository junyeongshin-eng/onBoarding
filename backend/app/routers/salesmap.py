...
async def _proxy_request(
    ...
    try:
        print(f"\n[Salesmap Proxy] ========================================")
        print(f"[Salesmap Proxy] Endpoint: {method} {SALESMAP_API_BASE}{endpoint}")
        print(f"[Salesmap Proxy] Request Body: {json.dumps(data, ensure_ascii=False, indent=2)}")
        ...
        async with httpx.AsyncClient() as client:
            ...
            print(f"[Salesmap Proxy] Response Status: {response.status_code}")
            print(f"[Salesmap Proxy] Response Body: {response.text[:1000]}")
